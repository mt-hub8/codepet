...
## V0.7 First-Run Onboarding + Dependency Doctor（已完成）

- 8 步首次启动引导：欢迎、基础功能、桌宠、提醒、Ollama、Agent CLI、任务监控、完成。
- 引导状态：`onboarding_completed`、`onboarding_completed_at`、`onboarding_skipped`。
- 依赖诊断页：基础依赖、本地 AI、Agent 工具、应用环境。
- 统一中文错误提示；只读检测，不自动安装依赖或拉取模型。
- 诊断信息可复制（脱敏）、可打开数据/日志目录。
- 设置页分组整理；首页轻量系统状态提示（最多 2 条）。

## V1.0 Public Beta

- 普通用户可下载、安装、配置，并在 10 分钟内用起来。
- **不再继续堆功能**，聚焦公开展示、稳定性、截图、demo 与 issue 模板。
